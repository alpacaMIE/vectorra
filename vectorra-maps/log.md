...
### Offline Prefetch Smoke Result Evidence Gate

Tightened the runtime device-smoke result checker so `offline-prefetch` and `cancel-prefetch` must prove cache state, completion, cancellation, and cleanup behavior instead of only action start/end markers.

Completed:

- Updated `tools/check-device-smoke-result.ps1` to require offline prefetch cache-before, progress, successful completion, cache-after, cancel request, canceled task state, and two cache-cleared log records.
- Refactored `tools/test-device-smoke-result-checker.ps1` valid log fixture into a reusable `$validLogText` and added offline prefetch/cancel/cache cleanup evidence.
- Added a missing offline cleanup failure fixture to the smoke result checker self-test.
- Extended `tools/test-device-smoke-contract.ps1` to guard offline prefetch result/cancel/cleanup requirements and the failure fixture.
- Updated `docs/beta/abi-device-matrix.md` and `docs/beta/android-1.0-acceptance.md` with the stronger offline prefetch smoke evidence requirement.
...
- PowerShell parser checks passed for the smoke checker, checker self-test, and smoke contract self-test.
- `tools/test-device-smoke-contract.ps1` passed.
- `tools/test-device-smoke-result-checker.ps1` passed, including the expected failure for a report missing offline prefetch cleanup evidence.
- `tools/check-android-acceptance.ps1 -GradleUserHome .\.gradle-agent-home` passed.
- Physical-device smoke remains blocked because adb still reports the attached device as `offline`.

### Sample Interaction Smoke Result Evidence Gate

Tightened the runtime device-smoke result checker so GeoJSON, Draw, and Location sample actions must prove their interaction behavior through logcat evidence.

Completed:

- Updated `tools/check-device-smoke-result.ps1` to require GeoJSON source/query logs, Draw request/query/clear logs, and Location indicator/follow/clear logs.
- Updated `tools/test-device-smoke-result-checker.ps1` valid fixture with GeoJSON/Draw/Location interaction evidence.
- Added a missing sample interaction query failure fixture to the smoke result checker self-test.
- Extended `tools/test-device-smoke-contract.ps1` to guard GeoJSON/Draw/Location log requirements and the failure fixture.
- Updated `docs/beta/abi-device-matrix.md` and `docs/beta/android-1.0-acceptance.md` with the stronger sample interaction smoke evidence requirement.
...
- PowerShell parser checks passed for the smoke checker, checker self-test, and smoke contract self-test.
- `tools/test-device-smoke-contract.ps1` passed.
- `tools/test-device-smoke-result-checker.ps1` passed, including the expected failure for a report missing the Draw center-query evidence.
- `tools/check-android-acceptance.ps1 -GradleUserHome .\.gradle-agent-home` passed.
- Physical-device smoke remains blocked because adb still reports the attached device as `offline`.

### MVT Runtime Smoke Result Evidence Gate

Tightened the runtime device-smoke result checker so MVT pan, hidden-layer, and remove/re-add actions must prove runtime render/query behavior through logcat evidence.

Completed:

- Updated `tools/check-device-smoke-result.ps1` to require regular MVT native render registration/application, pan query, hidden-layer no-features query, remove log, and re-add query evidence.
- Updated `tools/test-device-smoke-result-checker.ps1` valid fixture with regular MVT render/pan/hidden/re-add evidence.
- Added a missing MVT hidden no-features failure fixture to the smoke result checker self-test.
- Extended `tools/test-device-smoke-contract.ps1` to guard MVT pan/hidden/re-add log requirements and the failure fixture.
- Updated `docs/beta/abi-device-matrix.md` and `docs/beta/android-1.0-acceptance.md` with the stronger MVT runtime smoke evidence requirement.
...
- PowerShell parser checks passed for the smoke checker, checker self-test, and smoke contract self-test.
- `tools/test-device-smoke-contract.ps1` passed.
- `tools/test-device-smoke-result-checker.ps1` passed, including the expected failure for a report missing MVT hidden no-features evidence.
- `tools/check-android-acceptance.ps1 -GradleUserHome .\.gradle-agent-home` passed.
- Physical-device smoke remains blocked because adb still reports the attached device as `offline`.
...
- Added final screenshot visible-pixel validation to `tools/check-device-smoke-result.ps1`.
- Added a blank final screenshot failure fixture to `tools/test-device-smoke-result-checker.ps1`.
- Strengthened `tools/test-device-smoke-contract.ps1` so final screenshot visible-pixel coverage stays aligned.
- Updated Android acceptance, ABI/device matrix, release versioning, and 0.8.0 beta notes.
...
- PowerShell parser checks passed.
- `test-device-smoke-contract.ps1` passed.
- `test-device-smoke-result-checker.ps1` passed, including blank final screenshot rejection.
- Latest real emulator report `build/device-smoke/device-smoke-20260604-065746.txt` passed stricter checking with final screenshot `8456` visible samples and close-zoom 3D Tiles screenshot `2573` visible samples.
- `check-android-acceptance.ps1` passed and reported `Android local acceptance gate passed.`
