    ...
    */
    ...
    inline future<R> future<T>::then_dispatch(runtime& rt, F func, const context& con)
    {
        ...
        // lock the continuation and set it:
        {
            ...
            _shared->_continuation = [rt_ptr, func, copy_of_con, weak_shared, continuation_promise]() mutable
                {
                    ...
                    if (shared && shared->_ev.isSet())
                    {
                        ...
                        rt_ptr->dispatch(wrapper, copy_of_con);
                    }
                };
        }
    ...
    inline void future<T>::then_dispatch(runtime& rt, std::function<void(const T&)> func, const context& con)
    {
        // lock the continuation and set it:
        {
            ...
            _shared->_continuation = [rt_ptr, func, weak_shared, copy_of_con]() mutable
                {
                    auto shared = weak_shared.lock();
                    if (shared)
                    {
                        ...
                        rt_ptr->pool_dispatch(fire_and_forget_delegate, copy_of_con);
                    }
                };
        }
