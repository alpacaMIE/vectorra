...
ElevationLayer::createTileImplementation_internal(const TileKey& key, const IOOptions& io) const
{
    ...
    if (result.ok())
    {
...
#ifdef __ANDROID__
        static unsigned rawLogCount = 0u;
        if (rawLogCount++ < 24u)
        {
            auto raw = result.value().image();
            __android_log_print(
                ANDROID_LOG_INFO,
                "rocky_jni",
                "elevation raw key=%s valid=%d size=%ux%u format=%d",
                key.str().c_str(),
                result.value().valid() ? 1 : 0,
                raw ? raw->width() : 0u,
                raw ? raw->height() : 0u,
                raw ? static_cast<int>(raw->pixelFormat()) : -1);
        }
#endif
        ...
        {
#ifdef __ANDROID__
            static unsigned decodedLogCount = 0u;
            if (decodedLogCount++ < 24u)
            {
                Heightfield decoded(image);
                __android_log_print(
                    ANDROID_LOG_INFO,
                    "rocky_jni",
                    "elevation decoded key=%s size=%ux%u encoded=%d min=%.2f max=%.2f format=%d",
                    key.str().c_str(),
                    image->width(),
                    image->height(),
                    decoded.encoded() ? 1 : 0,
                    decoded.minHeight(),
                    decoded.maxHeight(),
                    static_cast<int>(image->pixelFormat()));
            }
#endif
        ...
        {
#ifdef __ANDROID__
            __android_log_print(ANDROID_LOG_WARN, "rocky_jni", "elevation decode failed key=%s", key.str().c_str());
#endif
...
ElevationLayer::createTileInKeyProfile(const TileKey& key, const IOOptions& io) const
{
    ...
    if (result.ok())
    {
        ...
        {
#ifdef __ANDROID__
            __android_log_print(ANDROID_LOG_WARN, "rocky_jni", "elevation result empty key=%s", key.str().c_str());
#endif
