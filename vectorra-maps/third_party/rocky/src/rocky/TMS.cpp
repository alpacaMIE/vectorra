...
TMS::Driver::read(const TileKey& key, bool invertY, bool isMapboxRGB, const URI::Context& context, const IOOptions& io) const
{
    ...
    if (tileMap.valid() && key.level <= tileMap.maxLevel)
    {
        ...
        {
#ifdef __ANDROID__
            static std::atomic_int fetchFailures = 0;
            int count = fetchFailures.fetch_add(1);
            if (count < 12)
            {
                __android_log_print(
                    ANDROID_LOG_WARN,
                    "rocky_jni",
                    "TMS tile fetch failed z=%u x=%u y=%u url=%s error=%s",
                    key.level,
                    key.x,
                    key.y,
                    imageURI.full().c_str(),
                    fetch.error().string().c_str());
            }
#endif
            return fetch.error();
        }
...
#ifdef __ANDROID__
        static std::atomic_int fetchSuccesses = 0;
        int fetchCount = fetchSuccesses.fetch_add(1);
        if (fetchCount < 12)
        {
            __android_log_print(
                ANDROID_LOG_INFO,
                "rocky_jni",
                "TMS tile fetched z=%u x=%u y=%u bytes=%zu type=%s url=%s",
                key.level,
                key.x,
                key.y,
                fetch->content.data.size(),
                fetch->content.type.c_str(),
                imageURI.full().c_str());
        }
#endif
        ...
        {
#ifdef __ANDROID__
            static std::atomic_int decodeFailures = 0;
            int count = decodeFailures.fetch_add(1);
            if (count < 12)
            {
                __android_log_print(
                    ANDROID_LOG_WARN,
                    "rocky_jni",
                    "TMS tile decode failed z=%u x=%u y=%u type=%s bytes=%zu error=%s",
                    key.level,
                    key.x,
                    key.y,
                    fetch->content.type.c_str(),
                    fetch->content.data.size(),
                    image_rr.error().string().c_str());
            }
#endif
...
#ifdef __ANDROID__
        static std::atomic_int decodeSuccesses = 0;
        int decodeCount = decodeSuccesses.fetch_add(1);
        if (image && decodeCount < 12)
        {
            __android_log_print(
                ANDROID_LOG_INFO,
                "rocky_jni",
                "TMS tile decoded z=%u x=%u y=%u image=%ux%u",
                key.level,
                key.x,
                key.y,
                image->width(),
