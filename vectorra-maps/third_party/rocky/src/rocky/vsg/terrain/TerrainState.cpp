...
TerrainState::updateRenderModel(const TileKey& key, const TerrainTileRenderModel& oldRenderModel, const TerrainTileModel& dataModel, VSGContext vsgcontext) const
{
...
#ifdef __ANDROID__
        static unsigned elevationRenderLogCount = 0u;
        if (elevationRenderLogCount++ < 48u)
        {
            __android_log_print(
                ANDROID_LOG_INFO,
                "rocky_jni",
                "render elevation key=%s size=%ux%u encoded=%d min=%.2f max=%.2f matrix=[%.4f %.4f %.4f %.4f]",
                dataModel.elevation.key.str().c_str(),
                renderModel.elevation.image ? renderModel.elevation.image->width() : 0u,
                renderModel.elevation.image ? renderModel.elevation.image->height() : 0u,
                hf.encoded() ? 1 : 0,
                renderModel.minHeight,
                renderModel.maxHeight,
                renderModel.elevation.matrix[0][0],
                renderModel.elevation.matrix[1][1],
                renderModel.elevation.matrix[3][0],
                renderModel.elevation.matrix[3][1]);
        }
#endif
    ...
    auto compileResult = vsgcontext->compile(descriptors.bind);
    if (!compileResult)
    {
        renderModel.valid = false;
        descriptors.bind = {};
        Log()->warn(
            "TerrainState failed to compile tile descriptor set for {}: {}",
            key.str(),
...
{    
    ...
    if (uniforms.terrainExaggeration != terrain.terrainExaggeration.value())
    {
        uniforms.terrainExaggeration = terrain.terrainExaggeration.value();
        _terrainDescriptors.data->dirty();
    }
