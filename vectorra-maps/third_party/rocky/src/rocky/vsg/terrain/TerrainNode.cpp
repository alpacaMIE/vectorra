...
TerrainProfileNode::createRootTiles(VSGContext vsgcontext)
{
    ...
    for (auto& key : keys)
    {
        ...
        if (!tile)
        {
            ...
            this->children.clear();
            return Failure(Failure::ResourceUnavailable, "Failed to compile terrain root tile");
        }
    ...
    auto cr = vsgcontext->compile(vsg::ref_ptr<TerrainProfileNode>(this));
#ifdef __ANDROID__
    __android_log_print(
        ANDROID_LOG_INFO,
        "rocky_jni",
        "TerrainProfileNode::createRootTiles compiled children=%zu result=%d requiresUpdate=%d containsPagedLOD=%d message=%s",
        children.size(),
        cr.result,
