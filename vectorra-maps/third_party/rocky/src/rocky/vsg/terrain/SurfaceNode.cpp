...
void
SurfaceNode::setTerrainExaggeration(float value)
{
    const auto next = std::clamp(value, 0.0f, 10.0f);
    if (next == _terrainExaggeration)
    {
        return;
    }

    _terrainExaggeration = next;
    _boundsDirty = true;
    recomputeBound();
}
...
SurfaceNode::recomputeBound()
{
    ...
    if (_elevationRaster)
    {
        ...
        ROCKY_SOFT_ASSERT_AND_RETURN(
            (!glm::epsilonEqual(scaleU, 0.0, EPSILON) && !glm::epsilonEqual(scaleV, 0.0, EPSILON)),
            worldBoundingSphere,
            _tilekey.str() <<);
        ...
        for (int i = 0; i < geom->verts->size(); ++i)
        {
            if (((int)geom->uvs->at(i).z & VERTEX_HAS_ELEVATION) == 0)
            {
                ...
                (*_proxyVerts)[i] = vert + norm * (h * _terrainExaggeration);
            }
            else
            {
