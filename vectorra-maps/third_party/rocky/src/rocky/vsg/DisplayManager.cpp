...
DisplayManager::configureTraits(vsg::WindowTraits* traits)
{
    ...
    if (!pd || pd->vk() == VK_NULL_HANDLE)
    {
        Log()->warn("No Vulkan physical device available during pre-window trait configuration; deferring device checks to window creation.");
        return;
    }
...
DisplayManager::addWindow(vsg::ref_ptr<vsg::WindowTraits> traits)
{
    ...
    __android_log_print(
        ANDROID_LOG_INFO,
        "rocky_jni",
        "DisplayManager::addWindow Window::create valid=%d width=%u height=%u",
        vsgWindow ? 1 : 0,
        traits->width,
        traits->height);
#endif
    ...
    __android_log_print(
        ANDROID_LOG_INFO,
        "rocky_jni",
        "DisplayManager::addWindow commandGraph valid=%d",
        commandGraph ? 1 : 0);
#endif
    ...
    __android_log_print(
        ANDROID_LOG_INFO,
        "rocky_jni",
        "DisplayManager::addWindow tracked windows=%zu",
        _windows.size());
#endif
