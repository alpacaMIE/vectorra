...
- 先修 SSE viewport 高度错误；该问题会放大加载、编译和渲染压力。
- P0 优先集中在 `sharedObjects`、`contentCache`、curl 选项和取消链路。
- P1 重点是 compile 线程迁移、磁盘缓存、下载 / 解码分池、元数据瘦身和 GPU 字节预算。
- P2 属于长期架构优化，包括 OBB 剔除、遍历去重、update 阶段选择、skip-LOD 和 HTTP/2。
...
这些改动可以放在同一次构建中，并在 HK 与 NLSC 两个数据集上对比效果。
