...
The checker self-test runs against synthetic local fixtures so the result verifier is known to catch crash-log, missing-action, invalid-screenshot, missing-device-metadata, empty-device-metadata, missing artifact byte record, blank-snapshot, and missing 3D Tiles zoom-snapshot failures before the device run.
