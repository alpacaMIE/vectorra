...
The local Android acceptance script runs this gate plus the SDK instrumentation APK build, ABI native-library content check, instrumentation APK no-native-library check, instrumentation APK checker self-test, device smoke script contract self-test, runtime smoke result checker self-test, emulator smoke gate contract self-test, and targeted MBTiles vector instrumentation runner contract self-test. The instrumentation APK checker self-test rejects missing, empty, and native-library APK fixtures. The smoke contract self-test keeps runner, checker, fixture, sample action names, and close-zoom 3D Tiles screenshot artifact coverage aligned. The smoke checker self-test validates a complete fixture and rejects crash logs, missing or out-of-order action markers, missing post-recreate snapshot markers or logs, missing or empty metadata, APK/ABI mismatches, missing or mismatched artifact records, invalid screenshots, missing close-zoom 3D Tiles screenshots, blank snapshots, and missing 3D Tiles close-zoom snapshots. The emulator smoke gate and MBTiles instrumentation runner self-tests keep the rerunnable device-facing scripts and docs aligned without requiring a device:
